...
public class QLBanHangModel {
    ...
    public String getMaOrder(String MaBan)
    {
        try 
        {
            String query = "SELECT MAORDER FROM ORDERS WHERE MABAN = ? AND TRANGTHAI = 'Chưa thanh toán'";
    ...
    public Vector getCTOrder(String MaOrder)
    {
        try 
        {
            String query = "SELECT * FROM CTORDERS, ORDERS WHERE CTORDERS.MAORDER = ORDERS.MAORDER AND ORDERS.MAORDER = '" + MaOrder + "' AND ORDERS.TRANGTHAI = '" + ConstantClass.ORDER_STATUS + "'";
            ResultSet rs = ConnectionManager.getInstance().executeQuery(query, null);
    ...
    public Vector getDSKMHienTai()
    {
        try 
        {
            String query = "SELECT * FROM KHUYENMAI WHERE sysdate BETWEEN NGAYBATDAU AND NGAYKETTHUC";
            Vector result = new Vector();
            ResultSet rs = ConnectionManager.getInstance().executeQuery(query, null);
            while (rs.next())
            {
    ...
    public String getMaKM(String TenKM)
    {
        try 
        {
            String query = "SELECT MAKM FROM KHUYENMAI WHERE TENKM = '" + TenKM + "'";
            ResultSet rs = ConnectionManager.getInstance().executeQuery(query, null);
    ...
    public long getTriGiaKM(String TenKM)
    {
        try 
        {
            String query = "SELECT TRIGIAKM FROM KHUYENMAI WHERE TENKM = '" + TenKM + "'";
    ...
    {
        String query = "UPDATE BAN SET TRANGTHAI = '" + TrangThai + "' WHERE MABAN = " + ID;
    ...
    {
        String query = "UPDATE ORDERS SET TRANGTHAI = '" + TrangThai + "' WHERE MAORDER = '" + MaOrder + "'";
    ...
    public boolean CapNhatDoUong(int SOLUONG,String MANL)
    {        
        String query = "update nguyenvatlieu set soluong="+SOLUONG+" where manvl='"+MANL+"'";
        ConnectionManager.getInstance().executeUpdate(query, null);
        return true;
    ...
    {
        String query = "INSERT INTO BAN(MABAN, TENBAN, TRANGTHAI) VALUES (" + String.valueOf(ID) + ", '" + TenBan + "', '" + TrangThai + "')";
